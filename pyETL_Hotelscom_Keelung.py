...
url = 'https://tw.hotels.com/search.do?resolved-location=CITY%3A1636762%3AUNKNOWN%3AUNKNOWN&destination-id=1636762&q-destination=%E5%9F%BA%E9%9A%86,%20%E5%8F%B0%E7%81%A3&q-check-in={}&q-check-out={}&q-rooms=1&q-room-0-adults=2&q-room-0-children=0'.format(start_date, end_date)
...
driver.get(url=url)
for page in range(1, 200):     # 模擬滾動200次
    driver.execute_script('window.scrollTo(0, document.body.scrollHeight);')
    time.sleep(2)     # 滾動後停留2秒讓網頁load近來
...
for i, title in enumerate(titles) :
    ...
    with open(r'./hotels_com/HC_Keelung/hotelcom_Keelung.json', 'a', encoding='utf-8') as w:
        w.write(json_data + '-----')

    with open(r'./hotels_com/HC_Keelung/hotelscom_Keelung_%s.txt' % (title.text), 'a', encoding='utf-8') as t:
        t.write(json_data)