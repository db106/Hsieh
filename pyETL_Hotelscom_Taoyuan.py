...
url = 'https://tw.hotels.com/search.do?resolved-location=CITY%3A1726032%3AUNKNOWN%3AUNKNOWN&destination-id=1726032&q-destination=%E6%A1%83%E5%9C%92,%20%E5%8F%B0%E7%81%A3&q-check-in={}&q-check-out={}&q-rooms=1&q-room-0-adults=2&q-room-0-children=0'.format(start_date, end_date)
...
for i, title in enumerate(titles) :
    ...
    grade_f = re.findall(r'\d+\.?\d', grade)[0]  # 使用正規表示式並取出list中的項目
    ...
    with open(r'./hotels_com/HC_Taoyuan/hotelcom_Taoyuan.json', 'a', encoding='utf-8') as w:
        w.write(json_data + '-----')

    with open(r'./hotels_com/HC_Taoyuan/hotelscom_Taoyuan_%s.txt' % (title.text), 'a', encoding='utf-8') as t:
        t.write(json_data)
